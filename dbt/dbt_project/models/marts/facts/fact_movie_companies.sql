...
-- depends_on: {{ ref('companies_snapshot') }}
-- depends_on: {{ ref('movies_snapshot') }}

{{
    config(
        materialized='incremental',
        unique_key='movie_company_sk',
...
/*
    Tabla de hechos para compañías de producción de películas
    ...
    - Ahora usamos companies_snapshot para implementar completamente SCD Type 2
*/
...
company_versions as (
    -- Extraemos las versiones de las compañías del snapshot
    select distinct
        -- Esta es la SK (surrogate key) que identifica cada versión de la compañía
        {{ dbt_utils.generate_surrogate_key(['company_name', 'dbt_valid_from']) }} as company_version_sk,
        -- Esta es la BK (business key) que identifica a la compañía a través de todas sus versiones
        company_name,
        company_id,
        _dlt_load_id,
        -- Estas son las fechas From/To que indican cuándo era válida cada versión
        dbt_valid_from,
        dbt_valid_to
    from {{ ref('companies_snapshot') }}
    where dbt_valid_to is null  -- Versión actual de la compañía
),

movie_company_relationships as (
    -- Obtenemos las relaciones compañía-película de la capa de staging
    select distinct
        id as movie_id,
...
select distinct
    ...
    {{ dbt_utils.generate_surrogate_key(['mcr.movie_id', 'mcr.company_name', 'md.dbt_valid_from', 'cv.dbt_valid_from']) }} as movie_company_sk,
    -- Incluimos la BK (business key) de la película
    mcr.movie_id,
    -- Incluimos la SK (surrogate key) de la versión de la película
    md.movie_version_sk,
    -- Incluimos la SK (surrogate key) de la versión de la compañía
    cv.company_version_sk,
    -- Incluimos el ID y nombre de la compañía
    cv.company_id,
    cv.company_name,
    ...
    cv.dbt_valid_from as company_valid_from,
    cv.dbt_valid_to as company_valid_to,
    -- Usamos _dlt_load_id para incrementalidad
    mcr._dlt_load_id
from movie_company_relationships mcr
join company_versions cv
    on cv.company_name = mcr.company_name
join movie_dates md
    on md.movie_id = mcr.movie_id
where mcr.company_name is not null and mcr.company_name != ''
{% if is_incremental() %}
    and (mcr._dlt_load_id > (select max(_dlt_load_id) from {{ this }})
         or not exists (
             select 1 from {{ this }}
             where movie_id = mcr.movie_id and company_name = mcr.company_name
         ))
{% endif %}
